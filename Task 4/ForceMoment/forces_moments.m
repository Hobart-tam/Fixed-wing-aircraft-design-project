...
function out = forces_moments(x, delta, wind, P)
    ...
    u_wg    = wind(4); % gust along body i-axis, week 4
    v_wg    = wind(5); % gust along body j-axis, week 4  
    w_wg    = wind(6); % gust along body k-axis, week 4
    ...
    R_v_b = [cos(theta)*cos(psi), cos(theta)*sin(psi), -sin(theta);
        ...
        cos(phi)*sin(theta)*cos(psi)+sin(phi)*sin(psi), cos(phi)*sin(theta)*sin(psi)-sin(phi)*sin(psi), cos(phi)*cos(theta)];
    ...
    V_a_b = [u;v;w]-V_w_b;%V_w_b is the overall wind data
    ...
    % Compute alpha (angle of attack) and beta (sldeslip angle)
